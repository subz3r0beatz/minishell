#!/usr/bin/env python3
"""
Minishell Universal Modern Dark GUI Test Harness (V2 PRO Backend)
Single-file self-contained Tkinter GUI test harness with stack backtrace symbol
resolution, readline valgrind suppressions, and isolated dual-mode execution.
Frontend: Original Tkinter Implementation
Backend: Asynchronous Subprocess Engine, Atomic IO, Strict PTY EOF Handling, XSS-Safe HTML.
"""
...
COLOR_BORDER        = "#313244"  
COLOR_FG_TEXT       = "#cdd6f4"  
COLOR_FG_MUTED      = "#a6adc8"  
...
COLOR_DIFF_ADD_BG   = "#283b2a"  
COLOR_DIFF_ADD_FG   = "#a6e3a1"
COLOR_DIFF_SUB_BG   = "#3f2229"  
...
#--- Embedded C Hook Source Code (Optimized Race Conditions) ---
MALLOC_HOOK_SRC = r"""
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
{
    if (real_malloc || atomic_exchange(&g_in_init, 1))
        return;
...
# =========================================================================
# BACKEND: Atomic File I/O Engine
# =========================================================================
...
def save_app_config(cfg):
    atomic_write_json(CONFIG_FILE_PATH, cfg)
...
def save_tests_to_file(tests_list):
    ...
    atomic_write_json(TESTS_FILE_PATH, clean_list)

def cleanup_test_artifacts():
    patterns = ["/tmp/ms_*", "/tmp/mini_*"]
    for pattern in glob.glob("/tmp/*"):
        if any(pattern.startswith(p.replace("*", "")) for p in patterns) and "minishell_gui_env_" not in pattern:
            try:
                if os.path.isdir(pattern): shutil.rmtree(pattern, ignore_errors=True)
                else: os.remove(pattern)
            except Exception:
                pass
...
def check_forbidden_functions(ms_path):
    ...
    try:
        ...
        for line in proc.stdout.splitlines():
            ...
            if not sym_clean or sym_clean.startswith("_") and sym_clean not in ALLOWED_42_FUNCTIONS:
                # Catch functions disguised with underscores
                pass
            
            if any(ign in sym_clean for ign in ignore_syms) or "faulty_malloc" in sym_clean:
                continue
            if sym_clean.lstrip('_') not in ALLOWED_42_FUNCTIONS and sym_clean not in ALLOWED_42_FUNCTIONS:
...
def run_norminette_check(ms_dir):
    norm_bin = shutil.which("norminette") or os.path.expanduser("~/.local/bin/norminette")
    if not os.path.exists(norm_bin):
...
# =========================================================================
# BACKEND: Subprocess & PTY Engine (Fully Asynchronous)
# =========================================================================
...
def run_interactive_session(executable, script_str, env=None, cwd=None, timeout=8):
    ...
    if "bash" in os.path.basename(executable):
        cmd.extend(["--norc", "--noprofile", "-i"])
    ...
    try:
        proc = subprocess.Popen(
            ...
            cwd=cwd, env=env or os.environ.copy(), start_new_session=True
        )
        os.close(slave)
        
        # Write payload all at once
        os.write(master, script_str.encode('utf-8'))
        
        out_b = bytearray()
        err_b = bytearray()
        read_fds = [master, proc.stderr.fileno()]
        start_t = time.time()
        
        while read_fds and (time.time() - start_t) < timeout:
            ...
            if proc.stderr.fileno() in r:
                try:
                    ...
                    if chunk:
                        ...
                        read_fds.remove(proc.stderr.fileno())
                except OSError:
                    read_fds.remove(proc.stderr.fileno())
        ...
        return out_b.decode('utf-8', errors='backslashreplace'), err_b.decode('utf-8', errors='backslashreplace'), proc.returncode
    finally:
        try: os.close(master)
        except: pass
...
def normalize_stdout(raw_stdout):
    ...
    clean = re.sub(r'^bash-[0-9\.]+\$[ \t]*', '', clean, flags=re.MULTILINE)
...
def normalize_stderr(raw_stderr, is_bash=False, bash_executable=None):
    ...
    if is_bash:
        clean = re.sub(r'^bash: cannot set terminal process group.*\n?', '', clean, flags=re.MULTILINE)
        clean = re.sub(r'^bash: no job control in this shell.*\n?', '', clean, flags=re.MULTILINE)
        if bash_executable:
...
def normalize_env_output(output_str):
    ...
    filtered = []
    ignore_prefixes = (
        "declare -x", "BASH=", "SHLVL=", "PWD=", "OLDPWD=", "_=", "LS_COLORS=", 
        "XDG=", "GLIBC=", "LD_=", "LD_PRELOAD=", "FAIL_MALLOC_", "TRACE_MALLOC=", 
...
def resolve_stack_trace(ms_path, raw_stderr):
    ...
    for line in stack_chunk.splitlines():
        ...
        # Support both glibc (+0x...) and musl/custom ([0x...]) backtrace symbols
        match_off = re.search(r'(?:\(\+(0x[0-9a-fA-F]+|[0-9a-fA-F]+)\)|\[(0x[0-9a-fA-F]+|[0-9a-fA-F]+)\])', line_str)
        if match_off:
            addresses.append(match_off.group(1) or match_off.group(2))
...
def execute_pty_signal_tests(ms_path, root_dir):
    ...
    try:
        ...
        os.close(m)
        if "130" in out2 or p.returncode == 0: logs.append("✔ [Interactive Prompt] Ctrl+C (SIGINT): OK")
        else: failures.append("Interactive Prompt: Ctrl+C did not set exit status to 130."); logs.append("✖ [Interactive Prompt] Ctrl+C: FAILED")
    except Exception as e: logs.append(f"✖ [Interactive Prompt] Ctrl+C test error: {e}")
...
def execute_single_test(test_item, ms_path, bash_path, hook_so_path, supp_file_path, opts, check_cancel=None):
    ...
    result = {
        "id": test_item["id"], "cmd": cmd_raw, "cat": test_item["cat"], "passed": True,
        "ms_out": "", "ms_err": "", "ms_code": 0, "ms_cwd": None, "ms_env": "",
        "bash_out": "", "bash_err": "", "bash_code": 0, "bash_cwd": None, "bash_env": "",
        "diff_text": "", "env_diff_text": "", "valgrind_log": "Not Run", "malloc_log": "Not Run", "signal_log": "Not Run",
        "duration_ms": 0.0, "pass1_duration_ms": 0.0, "pass2_duration_ms": 0.0, "pass3_duration_ms": 0.0,
        "valgrind_duration_ms": 0.0, "malloc_duration_ms": 0.0, "signals_duration_ms": 0.0,
        "interactive_out": "", "interactive_err": "", "interactive_code": 0, "interactive_cwd": None,
        "interactive_env": "", "interactive_diff": "", "failures": []
    }
    ...
    try:
        ...
        if opts.get("run_non_interactive", True):
            ...
            result["ms_out"], result["bash_out"] = normalize_stdout(raw_ms_out), normalize_stdout(raw_bash_out)
            result["ms_err"] = normalize_stderr(strip_hook_output(ms_err), is_bash=False)
            result["bash_err"] = normalize_stderr(bash_err, is_bash=True, bash_executable=bash_path)
            result["ms_code"], result["bash_code"] = ms_code, bash_code

            if test_item.get("flag_error", False):
                if not result["ms_err"] or ms_code == 0:
                    result["passed"] = False
                ...
                out_match = (normalize_env_output(result["ms_out"]) == normalize_env_output(result["bash_out"])) if "env" in cmd_raw or "export" in cmd_raw else (result["ms_out"] == result["bash_out"])
                err_match = (result["ms_err"] == result["bash_err"])
                code_match = (ms_code == bash_code)
                diff = difflib.unified_diff(result["bash_out"].splitlines(keepends=True), result["ms_out"].splitlines(keepends=True), fromfile="bash stdout", tofile="minishell stdout")
        ...
        # Pass B: Interactive Mode (PTY)
        if opts.get("run_interactive", True):
            if check_cancel and check_cancel(): return None
            
            raw_ms_int_out, raw_ms_int_err, ms_int_code = run_interactive_session(ms_path, f"{ms_cmd_str}\nexit\n", cwd=root_dir)
            raw_bash_int_out, raw_bash_int_err, bash_int_code = run_interactive_session(bash_path, f"{bash_cmd_str}\nexit\n", cwd=root_dir)

            ms_int_stdout = normalize_stdout(raw_ms_int_out)
            bash_int_stdout = normalize_stdout(raw_bash_int_out)

            if ms_int_stdout.startswith(ms_cmd_str): ms_int_stdout = ms_int_stdout[len(ms_cmd_str):].lstrip()
            if bash_int_stdout.startswith(bash_cmd_str): bash_int_stdout = bash_int_stdout[len(bash_cmd_str):].lstrip()

            ms_int_stdout = re.sub(r'\s+', ' ', ms_int_stdout).strip()
            bash_int_stdout = re.sub(r'\s+', ' ', bash_int_stdout).strip()
            ...
            result["interactive_out"], result["interactive_err"], result["interactive_code"] = ms_int_stdout, clean_ms_int_err, ms_int_code
            result["interactive_diff"] = "".join(difflib.unified_diff(bash_int_stdout.splitlines(keepends=True), ms_int_stdout.splitlines(keepends=True), fromfile="bash interactive stdout", tofile="minishell interactive stdout"))

            if test_item.get("flag_error", False):
                if not clean_ms_int_err or ms_int_code == 0:
                    result["passed"] = False
                    result["failures"].append("Interactive Flag Option Check: Expected non-zero exit code and error message on STDERR.")
            elif test_item.get("bash_cmp", True) and not opts.get("skip_bash", False):
                if not (ms_int_stdout == bash_int_stdout) or not (ms_int_code == bash_int_code) or not (clean_ms_int_err == clean_bash_int_err):
                    result["passed"] = False
                    result["failures"].append("Interactive Mode Pass: Output/Error mismatch")

        # Pass C & D: CWD & ENV
        if check_cancel and check_cancel(): return None
        t0 = time.perf_counter()
        ms_cwd_raw, _, _ = run_shell(f"{ms_cmd_str}\necho '__CWD_START__'\npwd -P\necho '__CWD_END__'\nexit $?\n", ms_path, cwd=root_dir)
        bash_cwd_raw, _, _ = run_bash(f"{bash_cmd_str}\necho '__CWD_START__'\npwd -P\necho '__CWD_END__'\nexit $?\n", bash_executable=bash_path, cwd=root_dir)
        result["pass2_duration_ms"] = round((time.perf_counter() - t0) * 1000, 2)
        result["ms_cwd"] = ms_cwd_raw.split("__CWD_START__")[1].split("__CWD_END__")[0].strip() if "__CWD_START__" in ms_cwd_raw else None
        result["bash_cwd"] = bash_cwd_raw.split("__CWD_START__")[1].split("__CWD_END__")[0].strip() if "__CWD_START__" in bash_cwd_raw else None

        if result["ms_cwd"] and result["bash_cwd"] and result["ms_cwd"] != result["bash_cwd"]:
            result["passed"] = False; result["failures"].append(f"CWD Mismatch: Minishell in '{result['ms_cwd']}', expected '{result['bash_cwd']}'")

        if opts.get("run_env_i", False):
            if check_cancel and check_cancel(): return None
            _, _, ms_code_i = run_shell(f"{ms_cmd_str}\nexit $?\n", ms_path, env={}, cwd=root_dir)
            if ms_code_i < 0 or ms_code_i in (134, 137, 139):
                result["passed"] = False; result["failures"].append(f"Env -i Pass: Minishell crashed/segfaulted under empty environment (Code {ms_code_i})")
        ...
        if not opts.get("skip_valgrind", False) and shutil.which("valgrind"):
            ...
            try:
                proc = subprocess.run(valgrind_cmd, input=f"{ms_cmd_str}\nexit $?\n", capture_output=True, text=True, cwd=root_dir, timeout=8)
                result["valgrind_duration_ms"] = round((time.perf_counter() - t0) * 1000, 2)
                result["valgrind_log"] = proc.stderr
                if proc.returncode == 99: result["passed"] = False; result["failures"].append("Valgrind: Memory leak or invalid memory access detected")
                if re.search(r"==\d+== FILE DESCRIPTORS: (?!0|1|2|3 open)\d+ open", proc.stderr): result["passed"] = False; result["failures"].append("Valgrind: File Descriptor leak detected")
            except Exception as e:
        ...
        if not opts.get("skip_malloc", False):
            ...
            _, ms_err_log, _ = run_shell(f"{ms_cmd_str}\nexit $?\n", ms_path, env=env, cwd=root_dir, timeout=1.5)
            ...
            if total_allocs > 0:
                ...
                for fail_idx in range(1, total_allocs + 1):
                    ...
                    _, err_m, code_m = run_shell(f"{ms_cmd_str}\nexit $?\n", ms_path, env=fail_env, cwd=root_dir, timeout=1.2)
                    ...
                    if code_m < 0 or code_m in (134, 137, 139):
                        ...
                        result["failures"].append("Malloc Fault: " + msg); malloc_fail_logs.append(msg); break
        ...
        if not opts.get("skip_signals", False):
            ...
            if sig_failures:
                result["passed"] = False
                for sf in sig_failures: result["failures"].append(sf)
...
# =========================================================================
# FRONTEND: GUI Class (Original Intact)
# =========================================================================
...
class MinishellTestGUI:
    def __init__(self, root):
        ...
        default_bash = cfg.get("bash_path", shutil.which("bash") or "/bin/bash")
    ...
    def toggle_inspector_view_mode(self, event=None):
        ...
        if sel:
            try:
                ...
                if test_item: self._update_inspector(test_item)
    ...
    def _debug_in_terminal(self):
        ...
        try: t_id = int(self.tree.item(sel[0])["values"][1])
        except (IndexError, ValueError): return "break"
        
        with self.tests_lock:
            test_item = next((t for t in self.tests_data if t["id"] == t_id), None)
        if not test_item: return "break"
        ...
        # Prioritize lightweight/modern Wayland terminals before X11 ones
        terminals = ["kitty", "alacritty", "foot", "wezterm", "gnome-terminal", "x-terminal-emulator", "xterm"]
...
time.sleep(1.0)
if payload: os.write(master_fd, payload.encode('utf-8'))

old_settings = None
if sys.stdin.isatty():
    try: old_settings = termios.tcgetattr(sys.stdin.fileno())
    except Exception: pass

try:
    if old_settings: tty.setraw(sys.stdin.fileno())
...
'''
        # Secure Payload Generation (Patching /tmp hijacking vulnerability)
        secure_dir = tempfile.mkdtemp(dir="/tmp")
        os.chmod(secure_dir, 0o700)
        runner_file = os.path.join(secure_dir, f"debug_runner_{test_item['id']}.py")
        
        with open(runner_file, "w", encoding="utf-8") as f:
            f.write(runner_code)

        run_script = f'python3 "{runner_file}"; rm -rf "{secure_dir}"; exec bash'
        try:
            if "gnome-terminal" in term: subprocess.Popen([term, "--", "bash", "-c", run_script])
            else: subprocess.Popen([term, "-e", "bash", "-c", run_script])
        except Exception as e: messagebox.showerror("Error", f"Failed to launch terminal debug session: {e}")
        return "break"
    ...
    def _run_compliance_audit(self):
        ...
        def write_audit():
            ...
            if not forbidden_list:
                ...
                for fn in forbidden_list: self.txt_audit.insert(tk.END, f"  • {fn}\n")
    ...
    def _export_html_report(self):
        with self.tests_lock: data_copy = [dict(t) for t in self.tests_data]
        ...
        html_doc = [
            "<!DOCTYPE html>", "<html><head><meta charset='utf-8'><title>Minishell Test Report</title>",
            "<style>",
            "body { font-family: 'Segoe UI', Arial, sans-serif; background-color: #1e1e2e; color: #cdd6f4; margin: 20px; }",
            "h1 { color: #89b4fa; }", ".summary { background-color: #252538; padding: 15px; border-radius: 8px; margin-bottom: 20px; }",
            ".pass { color: #a6e3a1; font-weight: bold; }", ".fail { color: #f38ba8; font-weight: bold; }",
            "table { width: 100%; border-collapse: collapse; margin-top: 10px; }",
            "th, td { padding: 10px; text-align: left; border-bottom: 1px solid #313244; }",
            "th { background-color: #252538; color: #89b4fa; }", "tr:nth-child(even) { background-color: #181825; }",
        ...
        for t in data_copy:
            ...
            # XSS Patch: html.escape applied to raw command text
            escaped_cmd = html.escape(t['cmd'])
            html_doc.append(
                f"<tr><td>{t['id']}</td><td>{t['cat']}</td><td><code>{escaped_cmd}</code></td>"
                f"<td class='{status_cls}'>{t['status']}</td><td>{dur}</td><td>{fails}</td></tr>"
            )

        html_doc.append("</table></body></html>")
        ...
        except Exception as e: messagebox.showerror("Export Error", f"Failed to save HTML report: {e}")
    ...
    def _move_test_up(self, event=None):
        ...
        with self.tests_lock:
            ...
            if idx is None or idx == 0: return "break"
        ...
        if target_uid is not None:
            for item in self.tree.get_children():
                try: row_id = int(self.tree.item(item)["values"][1])
                except (IndexError, ValueError): continue
                with self.tests_lock: matched = next((t for t in self.tests_data if t["id"] == row_id), None)
    ...
    def _move_test_down(self, event=None):
        ...
        with self.tests_lock:
            idx = next((i for i, t in enumerate(self.tests_data) if t["id"] == t_id), None)
            if idx is None or idx >= len(self.tests_data) - 1: return "break"
            target_uid = self.tests_data[idx]["_uid"]
            self.tests_data[idx], self.tests_data[idx + 1] = self.tests_data[idx + 1], self.tests_data[idx]
            self._reindex_tests_locked()
            save_tests_to_file(self.tests_data)
    ...
    def _navigate_tree(self, event):
        ...
        if not selection: target = children[0]
        ...
        self.tree.focus_set(); self.tree.selection_set(target); self.tree.focus(target); self.tree.see(target)
        return "break"
    ...
    def _focus_tree(self, event=None):
        ...
        if children and not self.tree.selection():
            self.tree.selection_set(children[0]); self.tree.see(children[0])
        return "break"

    def _handle_tab_navigation(self, event):
        if self._is_input_focused(): return None
        direction = -1 if event.keysym == "Left" else 1
        current = self.notebook.index(self.notebook.select())
        total = self.notebook.index("end")
        self.notebook.select((current + direction) % total)
        return "break"
    ...
    def _create_dark_text_tab(self, title, notebook, mono=False):
        ...
        txt = tk.Text(
            frame, wrap=tk.WORD if not mono else tk.NONE, font=("Consolas" if mono else "Segoe UI", 10),
            bg=COLOR_BG_INPUT, fg=COLOR_FG_TEXT, insertbackground=COLOR_FG_TEXT,
            selectbackground="#363a4f", selectforeground="#ffffff", relief="flat", bd=0, padx=10, pady=10, state=tk.DISABLED
        )
    ...
    def _recompile_minishell(self):
        ...
        self.lbl_status.config(text=f"Status: Recompiling in {make_dir} (make re)...", foreground=COLOR_WARN)
        self.root.update_idletasks()

        try:
            # Replaced "make" with "make re" to ensure clean build objects
            res = subprocess.run(["make", "re"], cwd=make_dir, capture_output=True, text=True)
            if res.returncode == 0:
                messagebox.showinfo("Recompile Success", f"Build succeeded in '{make_dir}'! 'make re' returned 0.\n\n" + (res.stdout[-600:] if res.stdout else "No output."))
    ...
    def _reset_selected_tests(self):
        ...
        with self.tests_lock:
            for t in self.tests_data:
                if t["selected"]: t["status"] = "PENDING"; t["result"] = None
        self._populate_tree()
        ...
        if sel:
            try:
                ...
                if t_item: self._update_inspector(t_item)
            except (IndexError, ValueError): pass
    ...
    def _populate_tree(self):
        ...
        sel = self.tree.selection()
        if sel:
            try: selected_id = int(self.tree.item(sel[0])["values"][1])
            except (IndexError, ValueError): pass
        ...
        for item in data_copy:
            ...
            tag = "PENDING"
            if item["status"] == "PASS": status_text = "✔ PASS"; tag = "PASS"
            elif item["status"] == "FAIL": status_text = "✖ FAIL"; tag = "FAIL"
            elif item["status"] in ("RUNNING", "QUEUED"): status_text = "⏳ RUNNING"; tag = "RUNNING"
    ...
    def _update_stats_bar(self):
        ...
        if self.is_running:
            if self.is_paused: self.lbl_status.config(text="Status: Paused", foreground=COLOR_WARN)
            else: self.lbl_status.config(text="Status: Executing Tests...", foreground=COLOR_WARN)
        else:
            self.lbl_status.config(text="Status: Ready", foreground=COLOR_ACCENT)

        self.lbl_metrics.config(text=f"Total: {total}  |  Selected: {selected}  |  Passed: {passed}  |  Failed: {failed}")
    ...
    def _on_tree_select(self, event):
        ...
        with self.tests_lock: test_item = next((t for t in self.tests_data if t["id"] == t_id), None)
        if test_item:
    ...
    def _select_failed_only(self):
        with self.tests_lock:
            for t in self.tests_data: t["selected"] = (t["status"] == "FAIL")
        self._populate_tree()

    def _select_passed_only(self):
        with self.tests_lock:
            for t in self.tests_data: t["selected"] = (t["status"] == "PASS")
        self._populate_tree()

    def _select_pending_only(self):
        with self.tests_lock:
            for t in self.tests_data: t["selected"] = (t["status"] == "PENDING")
        self._populate_tree()
    ...
    def _add_test_item(self):
        ...
        with self.tests_lock:
            self._uid_counter += 1
            self.tests_data.append({
                "_uid": self._uid_counter, "id": len(self.tests_data) + 1, "cat": cat, "cmd": cmd,
                "bash_cmp": self.edit_bash_cmp_var.get(), "flag_error": self.edit_flag_err_var.get(),
                "selected": True, "status": "PENDING", "result": None
            })
            self._reindex_tests_locked()
            save_tests_to_file(self.tests_data)
        self._populate_tree()
    ...
    def _update_test_item(self):
        ...
        with self.tests_lock:
            ...
            if test_item:
                ...
                test_item["status"] = "PENDING"; test_item["result"] = None
                save_tests_to_file(self.tests_data)
        self._populate_tree()
        if test_item: self._update_inspector(test_item)
        if self.is_running and not self.is_paused: self._ensure_worker_running()
    ...
    def _update_inspector(self, test_item):
        ...
        if not res or not isinstance(res, dict):
            self._write_read_only_text(self.txt_overview, lambda: self.txt_overview.insert(tk.END, f"Command: {test_item['cmd']}\nStatus: {test_item['status']}\n\nRun test to inspect output."))
        ...
        def write_overview():
            ov = [
                f"Command:  {res['cmd']}", f"Category: {res['cat']}", f"Result:   {'PASS' if res['passed'] else 'FAIL'}",
                ...
                f"\nWorking Directory (CWD) State:",
                f"  Minishell: {res['ms_cwd'] if res['ms_cwd'] else '(Not Captured)'}",
                f"  Bash:      {res['bash_cwd'] if res['bash_cwd'] else '(Not Captured)'}"
            ]
            if res["failures"]:
                ov.append("\nFailures / Mismatches Detected:")
                for f in res["failures"]: ov.append(f"  • {f}")
        ...
        def write_diff():
            ...
            else: self.txt_diff.insert(tk.END, f"✔ {mode_lbl} STDOUT matches Bash output perfectly.\n")
        ...
        def write_env():
            ...
            self.txt_env.insert(tk.END, f"\n=== MINISHELL FINAL ENVIRONMENT STATE ===\n", "info")
            self.txt_env.insert(tk.END, ms_env_norm + "\n")
            self.txt_env.insert(tk.END, "\n=== BASH FINAL ENVIRONMENT STATE ===\n", "info")
            self.txt_env.insert(tk.END, bash_env_norm + "\n")
        self._write_read_only_text(self.txt_env, write_env)

        self._write_read_only_text(self.txt_valgrind, lambda: self.txt_valgrind.insert(tk.END, f"Execution Duration (Valgrind Pass): {res.get('valgrind_duration_ms', 0)} ms\n\n" + res["valgrind_log"]))
        self._write_read_only_text(self.txt_malloc, lambda: self.txt_malloc.insert(tk.END, f"Execution Duration (Malloc Pass): {res.get('malloc_duration_ms', 0)} ms\n\n" + res["malloc_log"]))
        self._write_read_only_text(self.txt_signals, lambda: self.txt_signals.insert(tk.END, f"Execution Duration (Signal Pass): {res.get('signals_duration_ms', 0)} ms\n\n" + res["signal_log"]))

    def stop_tests(self):
        if not self.is_running and not self.is_paused: return
        self.stop_requested = True; self.pause_requested = False; self.is_paused = False
    ...
    def toggle_pause(self):
        ...
        if not self.is_paused:
            self.pause_requested = True; self.is_paused = True
            self.btn_pause.config(text="▶  Resume (Ctrl+P)")
            self.lbl_status.config(text="Status: Paused", foreground=COLOR_WARN)
            with self.tests_lock:
                for t in self.tests_data:
                    if t["status"] in ("RUNNING", "QUEUED"): t["status"] = "PENDING"; t["result"] = None
            self._populate_tree()
        else:
            self.is_paused = False; self.pause_requested = False
            self.btn_pause.config(text="⏸  Pause (Ctrl+P)")
            self.lbl_status.config(text="Status: Executing Tests...", foreground=COLOR_WARN)
            has_pending = False
            with self.tests_lock: has_pending = any(t["selected"] and t["status"] == "PENDING" for t in self.tests_data)
            if not has_pending: self._finish_run()
            else: self._ensure_worker_running()
    ...
    def run_tests(self):
        ...
        self.current_run_id += 1
        self.stop_requested = False; self.pause_requested = False; self.is_paused = False; self.is_running = True
    ...
    def _ensure_worker_running(self):
        if not self.is_running or self.is_paused or self.stop_requested: return
        ms_path = os.path.abspath(self.ms_path_var.get())
        bash_path = self.bash_path_var.get().strip()
        ...
        if self.worker_thread is None or not self.worker_thread.is_alive() or getattr(self.worker_thread, "run_id", None) != self.current_run_id:
            opts = {
                "skip_bash": not self.chk_bash.get(), "skip_valgrind": not self.chk_valgrind.get(),
                "skip_malloc": not self.chk_malloc.get(), "skip_signals": not self.chk_signals.get(),
                "run_env_i": self.chk_env_i.get(), "run_non_interactive": self.chk_non_interactive.get(),
    ...
    def _execute_test_wrapper(self, run_id, test_item, ms_path, bash_path, hook_so_path, supp_file_path, opts):
        def cancel_check():
            if not self.is_running or self.current_run_id != run_id or self.stop_requested or self.pause_requested: return True
            with self.tests_lock:
                existing = next((t for t in self.tests_data if t["_uid"] == test_item["_uid"]), None)
                if not existing or not existing["selected"]: return True
            return False

        if cancel_check(): return None
        self.msg_queue.put(("STARTING", run_id, test_item["_uid"], None))
        res = execute_single_test(test_item, ms_path, bash_path, hook_so_path, supp_file_path, opts, check_cancel=cancel_check)
        return res
            ...
            while self.is_running and self.current_run_id == run_id and not self.stop_requested and not self.pause_requested:
                ...
                for f in done_futures:
                    ...
                    try:
                        ...
                        else: self.msg_queue.put(("RESET_TEST", run_id, t_uid, None))
                    except Exception:
                        self.msg_queue.put(("RESET_TEST", run_id, t_uid, None))

                try: num_threads = max(1, min(32, self.jobs_var.get()))
                except Exception: num_threads = 4
                    ...
                    with self.tests_lock:
                        for t in self.tests_data:
                            if t["selected"] and t["status"] == "PENDING":
                                t["status"] = "QUEUED"; next_test = dict(t); break

                    if next_test:
                        f = executor.submit(self._execute_test_wrapper, run_id, next_test, ms_path, bash_path, hook_so_path, supp_file_path, opts)
                ...
                if not active_futures:
                    has_pending = False
                    with self.tests_lock: has_pending = any(t["selected"] and t["status"] == "PENDING" for t in self.tests_data)
                    if not has_pending: break
                time.sleep(0.02)

            for f in list(active_futures.keys()): f.cancel()
        self.msg_queue.put(("BATCH_DONE", run_id, None, None))

    def _finish_run(self):
        self.is_running = False; self.is_paused = False; self.stop_requested = False; self.pause_requested = False
    ...
    def _update_overall_progress(self):
        with self.tests_lock:
            ...
            if selected_tests:
                ...
                self.progress_var.set((completed_count / len(selected_tests)) * 100)

    def _poll_queue(self):
        try:
            while True:
                msg_type, run_id, t_uid, res = self.msg_queue.get_nowait()
                if run_id != self.current_run_id: continue
                ...
                if msg_type == "STARTING":
                    ...
                    if not self.stop_requested and not self.pause_requested and isinstance(res, dict):
                        with self.tests_lock:
                            for t in self.tests_data:
                                if t["_uid"] == t_uid:
                                    t["result"] = res; t["status"] = "PASS" if res.get("passed", False) else "FAIL"
                                    break
                        ...
                        if sel:
                            try:
                                ...
                                with self.tests_lock: t_item = next((t for t in self.tests_data if t["id"] == sel_id), None)
                                if t_item and t_item["_uid"] == t_uid: self._update_inspector(t_item)
                            except (IndexError, ValueError): pass
                    ...
                    if self.is_paused:
                        ...
                        has_pending = False
                        with self.tests_lock: has_pending = any(t["selected"] and t["status"] == "PENDING" for t in self.tests_data)
                        if has_pending and self.is_running and not self.stop_requested: self._ensure_worker_running()
...
def main_cli(args):
    ...
    for idx, item in enumerate(raw_tests):
        tests_data.append({
            "id": idx + 1, "cat": item.get("cat", "Custom"), "cmd": item.get("cmd", ""),
            "bash_cmp": item.get("bash_cmp", True), "flag_error": item.get("flag_error", False)
        })

    opts = {
        "skip_bash": False, "skip_valgrind": False, "skip_malloc": False, "skip_signals": False,
        "run_env_i": True, "run_non_interactive": True, "run_interactive": True
    }
        ...
        for future in as_completed(futures):
            ...
            if res and res["passed"]:
                ...
                if res and res["failures"]:
                    for f in res["failures"]: print(f"      ├─ {f}")

    print("\n=== Compliance Audit Summary ===")
    forbidden = check_forbidden_functions(ms_path)
    if not forbidden: print("  ✔ External functions audit: Passed (All symbols whitelisted)")
    else: print(f"  ✖ Forbidden symbols detected: {', '.join(forbidden)}")
