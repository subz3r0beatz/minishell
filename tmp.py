...
def run_interactive_session(executable, script_str, env=None, cwd=None, timeout=8):
    ...
    try:
        proc = subprocess.Popen(
            cmd,
            stdin=slave_out, stdout=slave_out, stderr=slave_err,
        ...
        out_b = bytearray()
        err_b = bytearray()
        read_fds = [master_out, master_err]
        
        for line in script_str.splitlines(keepends=True):
            os.write(master_out, line.encode('utf-8'))
            ...
            while True:
                ...
                if master_out in r:
                    try:
                        ...
                        if chunk: out_b.extend(chunk)
                        else: read_fds.remove(master_out)
                    except OSError:
                        if master_out in read_fds: read_fds.remove(master_out)
                if master_err in r:
                    try:
                        chunk = os.read(master_err, 4096)
                        if chunk: err_b.extend(chunk)
                        else: read_fds.remove(master_err)
                    except OSError:
                        if master_err in read_fds: read_fds.remove(master_err)
        
        start_t = time.time()
        while time.time() - start_t < 1.0 and read_fds:
        ...
        try: os.close(master_out)
        except Exception: pass
        try: os.close(master_err)
        except Exception: pass
...
def execute_single_test(test_item, ms_path, bash_path, hook_so_path, supp_file_path, opts, check_cancel=None):
    ...
    try:
        ...
        # Separated from State Extraction for clean raw IO testing
        # =========================================================================
        if opts.get("run_interactive", True):
            if is_cancelled(): return None
            
            interactive_script = f"{ms_cmd_str}\nexit\n"
            bash_interactive_script = f"{bash_cmd_str}\nexit\n"
            ...
            if ms_int_stdout.startswith(ms_cmd_str):
                ms_int_stdout = ms_int_stdout[len(ms_cmd_str):].lstrip()
            
            if bash_int_stdout.startswith(bash_cmd_str):
                bash_int_stdout = bash_int_stdout[len(bash_cmd_str):].lstrip()

            # 2. Normalize whitespace (collapses Bash's 0-column vertical list to match horizontal lists)
            ...
            result["interactive_code"] = ms_int_code
            result["interactive_cwd"] = None
            result["interactive_env"] = None
            ...
            if test_item.get("flag_error", False):
                ...
                int_code_match = (ms_int_code == bash_int_code)

                if not int_out_match or not int_code_match or not int_err_match:
                    result["passed"] = False
                    int_reasons = []
                    ...
                    if not int_code_match: int_reasons.append(f"Interactive exit status mismatch (minishell={ms_int_code}, bash={bash_int_code})")
                    result["failures"].append("Interactive Mode Pass: " + ", ".join(int_reasons))

        # =========================================================================
        # PASS C & D: CWD & ENV Non-Interactive State Probes 
        # Extract states safely without terminal echoes
        # =========================================================================
        if is_cancelled(): return None
        t0 = time.perf_counter()
