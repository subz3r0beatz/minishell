...
/*   Updated: 2026/07/13 20:12:01 by fldumas-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	size_t max_uints[4])
{
	...
	if (!parse_flag(&(*args)[*i][*j], "chdir", j))
		ret = parse_long_chdir_path(*args, flags, i, j);
	else if (!parse_flag(&(*args)[*i][*j], "argv0", j))
		ret = parse_long_argv0(*args, flags, i, j);
	else if (!parse_flag(&(*args)[*i][*j], "split-string", j))
		ret = handle_long_split(args, i, j, max_uints[3]);
	else if (!parse_flag(&(*args)[*i][*j], "unset", j))
		ret = handle_long_unset(*args, exported, max_uints);
	else
